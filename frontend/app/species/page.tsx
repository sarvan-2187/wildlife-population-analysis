...
export default function SpeciesPage() {
  ...
  const totalForecastEvaluated = Number(integratedPredictions?.metrics?.species_evaluated || 0);
  const totalForecastSuccessful = Number(integratedPredictions?.metrics?.species_successful || 0);
  const fallbackForecasts = Number(integratedPredictions?.metrics?.species_fallback_predictions || 0);
  const fullForecasts = Number(integratedPredictions?.metrics?.species_full_predictions || 0);
  const forecastCoveragePct = totalForecastEvaluated
    ? ((totalForecastSuccessful / totalForecastEvaluated) * 100).toFixed(1)
    : "0.0";
  const uniqueRegionCount = new Set(species.map((s) => s.region).filter(Boolean)).size;
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 md:px-10 py-8 md:py-10 space-y-6 mb-12">
      ...
      <section className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "Declining", value: trackedDeclining.toLocaleString(), icon: TrendingDown },
          {
            ...
            value: `${forecastCoveragePct}% `,
            icon: LineChartIcon,
          },
          { label: "Global footprint", value: `${uniqueRegionCount.toLocaleString()} regions`, icon: Globe },
        ].map((card) => {
      ...
      <section className="grid sm:grid-cols-2 gap-4">
        {/* <article className="glass-panel rounded-2xl p-5 glow-card">
          <p className="text-xs uppercase tracking-[0.14em] text-[#8ca8a1]">Full model forecasts</p>
          <p className="mt-2 text-3xl font-semibold text-[#e8fff7]">{fullForecasts.toLocaleString()}</p>
          <p className="mt-1 text-sm text-[#9bb9b1]">Generated by classifier + time-series model</p>
        </article> */}
        {/* <article className="glass-panel rounded-2xl p-5 glow-card">
          <p className="text-xs uppercase tracking-[0.14em] text-[#8ca8a1]">Fallback forecasts</p>
          <p className="mt-2 text-3xl font-semibold text-[#e8fff7]">{fallbackForecasts.toLocaleString()}</p>
          <p className="mt-1 text-sm text-[#9bb9b1]">Status-based inference where model fit was unavailable</p>
        </article> */}
      </section>
          ...
          <table className="w-full min-w-245 text-left">
            ...
            <tbody>
              {paginatedSpecies.map((item, idx) => {
                ...
                const isFallbackForecast = prediction?.integration?.recommendation === "FALLBACK_FORECAST";
                const forecastColor =
                    ...
                    <td className="px-5 py-4 text-sm">
                      {forecastTrend ? (
                        <div className="space-y-1">
                          ...
                          {/* {isFallbackForecast && (
                            <p className="text-[11px] uppercase tracking-widest text-[#f7c46a]">Fallback</p>
                          )} */}
                        </div>
                      ) : (
    ...
    <div className="fixed inset-0 z-9999 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div className="glass-panel rounded-3xl p-5 md:p-6 w-full max-w-5xl max-h-[92dvh] overflow-y-auto border border-[#345260]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-5 rounded-2xl p-4 bg-[#0b1b26] border border-[#31505d]">
          <h4 className="text-lg text-[#e9fff8] inline-flex items-center gap-2">
            <LineChartIcon className="w-5 h-5" />
            Forecast details and confidence
          </h4>
          <div className="mt-3 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {forecastYears.map((year: number, idx: number) => {
              const ciData = forecastData?.confidence_intervals?.[String(year)];
              return (
                <div key={year} className="rounded-xl p-4 bg-[#0f2430] border border-[#31505b]">
                  <p className="text-xs text-[#90aea6] uppercase tracking-[0.12em] font-semibold">{year}</p>
                  <p className="text-[#defaf1] font-semibold mt-2 text-lg">{Number(forecastPops[idx] || 0).toLocaleString()}</p>
                  {ciData && (
                    <div className="mt-2 text-xs text-[#8fb5ac] space-y-1">
                      <p>
                        CI: {Number(ciData.lower || 0).toLocaleString()} — {Number(ciData.upper || 0).toLocaleString()}
                      </p>
                    </div>
                  )}
                  {forecastTrend?.growth_rate_inference !== undefined && idx === 0 && (
                    <p className="mt-2 text-xs text-[#8fb5ac]">
                      Growth: <span className={forecastTrend.growth_rate_inference >= 0 ? "text-[#86efcc]" : "text-[#ffb29f]"}>
                        {forecastTrend.growth_rate_inference > 0 ? "+" : ""}{forecastTrend.growth_rate_inference.toFixed(2)}%
                      </span>
                    </p>
                  )}
                </div>
